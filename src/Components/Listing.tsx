...
import { getAllCars } from "../services/carApis";
import { useDispatch, useSelector } from "react-redux";
import { RootState } from "../redux/store";
import { setAllCars } from "../redux/carSlice";
import { Car } from "../types/shared";
...
export const Listing = () => {
  ...
  const cars = useSelector((state: RootState) => state.carSlice.cars);
  const filteredCars = useSelector(
    (state: RootState) => state.carSlice.filterdCars
  );
  useEffect(() => {
    getAllCars()
      .then((res) => {
      ...
      .then((res) => {
        dispacth(setAllCars(res));
      });
  }, []);
  const carTypeFilters = (type: string): carFilters[] => {
    let filtersMap = new Map();
    let carTypeFilters: carFilters[] = [];
    if (cars.length) {
      cars.forEach((elem: Car) => {
        if (
          !filtersMap.has(type === "carType" ? elem.carType : elem.transmission)
        ) {
          if (type === "carType") {
            carTypeFilters.push({
              type: elem.carType,
            ...
            carTypeFilters.push({
              type: elem.transmission,
            });
          }
        }
        type === "carType"
          ? filtersMap.set(elem.carType, 1)
          : filtersMap.set(elem.transmission, 1);
      });
    }
    return carTypeFilters;
  };
  ...
  return (
    <div className="w-10/12 ml-auto">
      ...
      <div className="bg-gray-100 p-8 flex flex-col gap-3">
        <div className="font-sans text-3xl text-gray-900 font-bold">
          Booking
        </div>
        <div className="flex">
          <div className="flex flex-grow justify-start items-start gap-4">
            <SelectInput
              filterType="carType"
              data={carTypeFilters("carType")}
            />
            <SelectInput
              filterType="transmission"
              data={carTypeFilters("transmission")}
            ></SelectInput>
          </div>
